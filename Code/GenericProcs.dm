...
proc/NameGuard(var/NewName)
	if(!NewName)	return
	var/list/ReservedNames=list("MaXxiii","Ziddy","MaXxiii","Ziddeh","Ziddi")
...
obj/Supplemental
	DamageNum
		icon='Icons/Other/DamageNums.dmi'
		density=0;layer=101

var/GainText='Icons/HUD/Text.dmi'+rgb(255,255,255)

proc/DamageShow(var/atom/A,var/Damage,var/DamageIcon='Icons/Other/DamageNums.dmi')
	if(!A)	return
	if(istext(Damage))
		var/obj/O=DamageNums[1]
		DamageNums-=O;DamageNums+=O
		if(O.icon!='Icons/Other/DamageNums.dmi')	O.icon='Icons/Other/DamageNums.dmi'
...
atom/proc/AddName(var/Name2Add)
	...
	if(ismob(src) && src:Subscriber && src:DisplayNameColor!=initial(src:DisplayNameColor))
		DisplayNameIcon='Icons/HUD/Text.dmi';DisplayNameIcon+=src:DisplayNameColor
	var/PixelSpace=6
	...
	while(1)
		...
		if(!ismob(src) || !src:client)	NL.icon='Icons/HUD/TextNPC.dmi'
		src.overlays+=NL
