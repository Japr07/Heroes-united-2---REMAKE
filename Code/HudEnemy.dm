obj/HUD
	...
	EnemyPlBar
		layer=11
		icon='Icons/Other/EnemyPlBar1.dmi'
		screen_loc="13:14,17:2"
	EnemyKiBar
		layer=11
		icon='Icons/Other/EnemyKiBars.dmi'
	...
	for(var/client/C in src.ControlClients)
		...
		for(var/i=1;i<=4;i++)
			if(!C.mob.EnemyPlBars[i])
				...
				if(i==2 || i==3)	PLB.icon='Icons/Other/EnemyPlBar2.dmi'
				if(i==4)	PLB.icon='Icons/Other/EnemyPlBar3.dmi'
				C.screen+=PLB
