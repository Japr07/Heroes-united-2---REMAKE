...
mob/proc/BeamHit(var/mob/Beamer,var/DamageMult=1)
	for(var/i=1;i<=5;i++)
		...
		PlaySound(view(src),pick('Sound/Effects/Hit0.ogg','Sound/Effects/Hit1.ogg','Sound/Effects/Hit2.ogg','Sound/Effects/Hit3.ogg','Sound/Effects/Hit4.ogg'))
	...
	spawn(5)
		...
		while(BeamHead && CounterBeamer && MyGetDist(BeamHead.loc,CounterBeamer.loc)>1)
			if(src.StartButtonCombo(CanFail=1))
				...
				PlaySound(view(BeamHead),'Sound/Effects/EnergyStart.ogg')
...
atom/proc/AddBBKOverlays()
	switch(src.dir)
		if(NORTH)
			var/obj/O=new;O.icon='Icons/Other/Effects.dmi';O.layer=FLOAT_LAYER
