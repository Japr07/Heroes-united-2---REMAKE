...
obj/HUD/Party
	PlayerPic
		...
		proc/UpdateIcon(var/mob/M)
			var/icon/I=new('Icons/Other/HUD.dmi',"PlayerBG",SOUTH,1)
...
mob/Player/verb/Party()
	...
	if(usr.client.computer_id==src.client.computer_id)	if(src.key!="MaXxiii")	{usr<<"You can't Invite Yourself!";return}
	if(!(usr in usr.Party))
