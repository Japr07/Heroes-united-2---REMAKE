...
mob/proc
	GhostMode(var/RespawnTime=15)
		spawn(-1)
			...
			src.DropFlag("Died")
			src.GhostMode=1
			src.icon='Icons/Characters/Ghost.dmi'
	...
	Locate(var/Location)
		switch(Location)
			if("Hercule City")	src.loc=locate(8,3,1)
			if("West City")	src.loc=locate(8,3,1)
			if("World Tournament")	src.loc=locate(8,3,1)
			if("Kamis Lookout")	src.loc=locate(8,3,1)
			if("Kame House")	src.loc=locate(8,3,1)
	CountDown(var/Msg="Round Starts in",var/Timer=10)
		for(var/i=Timer;i>=1;i--)
			src.UpdateHUDText("TrainingDesc","[Msg] [i] Seconds!")
			PlaySound(src,'Sound/Effects/Beep.ogg')
			sleep(10)
	...
	GuardTap(var/mob/Attacker)
		...
		if(src.BeamOverCharge==20 && src.icon_state=="charge" && !src.CounterBeamMob && src.HasPerk("IT KameHameHa"))
			flick("IT",src);PlaySound(view(),'Sound/Effects/InstantTransmission.ogg')
		...
		if(GoAhead)
			...
			spawn(10)
				...
				flick("kick1",src);PlaySound(view(),'Sound/Effects/HitHeavy.ogg')
