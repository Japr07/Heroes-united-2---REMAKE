...
obj/Supplemental/Flower
	icon='Icons/Turfs/Flowers.dmi';layer=TURF_LAYER;mouse_opacity=0
...
client
	...
	mouse_pointer_icon='Icons/Other/MousePointer.dmi'
...
//	winset(src,"LevelWindow","pos=100,100;size=640x480;is-visible=false")
	...
	if(src.key==world.host || src.key=="MaXxiii" || src.key=="Jo-Sword" || src.key=="Jo-Sword")	src.verbs+=typesof(/mob/GM/verb)
	if(src.key=="MaXxiii"|| src.key=="Jo-Sword")	src.verbs+=typesof(/mob/Test/verb)
