...
mob/verb
	CounterAttack(/**/)
		...
		if(src.HitStun)	for(var/mob/M in oview(1))
			if(M.Attacking && src.CanPVP(M))
				...
				PlaySound(view(),'Sound/Effects/HitHeavy.ogg')
		if(!src.CanAct())	return
		src.Countering=1
		spawn(6)	src.Countering=0
		if(src.CounterBeam())	return
		flick("punch2",src)
		PlaySound(view(),pick('Sound/Effects/Swing1.ogg','Sound/Effects/Swing2.ogg'))
	...
	FullTransform()
		...
		if(NewTrans && src.CurTrans<NewLevel)
			...
			PlaySound(src,'Sound/Effects/PowerUpEnd.ogg')
	Transform()
		set hidden=1
		src=src.GetFusionMob()
		if(!src.CanAct())	return
		...
		if(src.Character.Transes.len>src.CurTrans)
			if(!src.HasKiPercent(60))
				PlaySound(view(),'Sound/Effects/PowerUpEnd.ogg');return
			var/datum/TransDatum/NextTrans=src.Character.Transes[src.CurTrans+1]
			if(src.MaxPL<NextTrans.ReqPL)
				src<<"Requires a Power Level of [FullNum(NextTrans.ReqPL)]"
				PlaySound(src,'Sound/Effects/PowerUpEnd.ogg');return
			src.CurTrans+=1
			flick("transform",src)
			src.ApplyTransDatum()
			...
			PlaySound(view(),'Sound/Effects/EnergyStart.ogg')
			sleep(8)
			src.UpdateFaceHUD()
			spawn()	src.ScreenShake()
			PlaySound(view(),'Sound/Effects/ActivateSpecial.ogg')
			for(var/mob/M in oview(1))	if(M.icon_state!="koed")
				if(src.CanPVP(M))
					M.HitStun(5,src)
					src.KnockBack(M,src)
					PlaySound(view(),pick('Sound/Effects/HitLight1.ogg','Sound/Effects/HitLight2.ogg'))
	Attack(/**/)
		...
		for(var/mob/M in GetStep)	if(M.density==src.density)
			...
			M.dir=get_dir(M,src)
			var/Sound2Play='Sound/Effects/Hit3.ogg'
			switch(src.ComboCount)
				if(1)	Sound2Play='Sound/Effects/Hit3.ogg'
				if(2,4)	Sound2Play='Sound/Effects/Hit0.ogg'
				if(3)	Sound2Play='Sound/Effects/Hit4.ogg'
				else	Sound2Play=pick('Sound/Effects/Hit3.ogg','Sound/Effects/Hit0.ogg','Sound/Effects/Hit4.ogg')
			if(src.ComboCount>=MaxCombo)	Sound2Play='Sound/Effects/HitHeavy.ogg'
			...
			if(M.Attacking)	ShowEffect(M,"HyperCombat",16,16)
				...
				if(src.HasPerk("Pickpocket"))	if(M.Zenie>=10)
					PlaySound(view(M),'Sound/Effects/Rupee.ogg')
	...
	StrongAttackRelease()
		...
		flick("punch1",src);PlaySound(view(),pick('Sound/Effects/Swing1.ogg','Sound/Effects/Swing2.ogg'))
	...
	PowerUp(/**/)
		...
		while(src.PoweringUp)
			...
			if(src.icon_state!="powerup")
				...
				PlaySound(view(),'Sound/Effects/EnergyStart.ogg',channel=7)
				PlaySound(view(),'Sound/Effects/EnergyLoop.ogg',repeat=1,channel=8)
	...
	Guard(var/ShiftKey as null|num)
		...
		if(src.icon_state=="KnockBack")
			...
			PlaySound(view(),'Sound/Effects/ThrowStop.ogg')
	...
	KiBlast()
		...
		if(!Damage)	{PlaySound(src,'Sound/Effects/PowerUpEnd.ogg');return}
		src.Charging=1
		spawn(2)	src.Charging=0
		PlaySound(view(),pick('Sound/Effects/KiBlast1.ogg','Sound/Effects/KiBlast2.ogg'))
	...
	ChargeBeam()
		...
		if(!src.CanAct())	return
		if(src.CounterBeam())	return
		if(!src.UseKiPercent(20))	{PlaySound(src,'Sound/Effects/PowerUpEnd.ogg');return}
		if(src.HasPerk("Flash Finish"))	src.icon_state="charge"
			...
			PlaySound(view(),'Sound/Effects/EnergyLoop.ogg',repeat=1,channel=8,VolChannel="Effect")
