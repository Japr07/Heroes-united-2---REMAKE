...
mob/proc/ShadowAttack(var/Dir)
	...
	if(Dir==src.SparCmds[1])
		...
		PlaySound(src,'Sound/Effects/NoBuzz.ogg')
...
mob/proc
	...
	FocusTrainingProc(var/atom/SourceAtom)
		...
		if(MyGetDist(src,SourceAtom)>world.view)	return
		src.ShowTutorial(Tutorials["Training Exp"])
		PlaySound(view(src),'Sound/Effects/EnergyStart.ogg')
		src.CompleteTutorial("Additional Training")
		...
		proc/Attacked(var/mob/Player/M)
			...
			if(!M.Training)
				...
				if(M.TrainingStrike)
					...
					PlaySound(M,'Sound/Effects/NoBuzz.ogg')
			...
			else	PlaySound(view(),'Sound/Effects/Hit3.ogg')
		Speed_Bag
			icon_state="SpeedBag"
		Punching_Bag
